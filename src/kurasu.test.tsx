...
it("should allow using prop argument in withClasses Component", () => {
  ...
  const TestComponent = kurasu(TestWrappedComponent, (props) => props.someProp);
...
it("should change the classes when props are changed, for the clsx function", () => {
  /**
   * This button has a status: isOpen. When clicking the button,
   * this status will get toggled by the parent component
   * @param props
   ...
   * will have a is-open class.
   */
  const TestComponent = kurasu(TestButtonComponent, (props, clsx) =>
    clsx(props.isOpen && "is-open")
  );
...
it("should not forward unknown props to elements", () => {
  const TestComponent = kurasu<{ isOpen: boolean }>("button", "");
  render(<TestComponent isOpen={true} />);
  expect(screen.getByRole("button")).not.toHaveAttribute("isOpen");
});

it("should forward unknown props to components", () => {
  const Button = vi.fn((props: { isOpen: boolean; className: string }) => (
    <button>test</button>
  ));
...
it("should override tailwind props", () => {
  const TestComponent = kurasu("button", "p-4");
  render(<TestComponent className="p-8">test content</TestComponent>);

  expect(screen.getByRole("button")).toHaveClass("p-8");
  expect(screen.getByRole("button")).not.toHaveClass("p-4");
});
